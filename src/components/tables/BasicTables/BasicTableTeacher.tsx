...
export default function BasicTableTeacher() {
  ...
  const handleToggleMask = (id: string) => {
    const teacher = tableData.find(item => item.id === id);
    if (teacher) {
      setAuthAction(teacher.isMasked ? "unmask" : "mask");
      setTargetRowId(id);
      openAuthModal();
    }
  };
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authAction === "unmask" && targetRowId) {
        ...
        alert("Teacher unmasked successfully.");
      ...
      } else if (authAction === "edit" && selectedTeacher) {
        openEditModal(); // Proceed to edit modal after auth
        alert("Proceeding to edit teacher.");
      }
    } catch (error) {
      ...
      closeAuthModal(); // Automatically close auth modal after action
      setAuthPassword("");
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nameParts = editTeacherName.trim().split(" ");
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <>
                  ...
                  {/* Verified Accounts */}
                  {verifiedData.map((teacher) => (
                    <TableRow key={teacher.id}>
                      <TableCell className="px-5 py-4 sm:px-6 text-start">
                        <div className="flex items-center gap-3">
                          <div>
                            <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                              {teacher.isMasked ? "••••••••" : teacher.email}
                            </span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">
                        {teacher.isMasked ? "••••••••" : teacher.teacherName}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">
                        <div className="flex -space-x-2">
                          {teacher.isMasked ? "••••••••" : teacher.subject}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                        <div className="flex space-x-2">
                          <Button
                            size="md"
                            variant="outline"
                            onClick={() => handleToggleMask(teacher.id)}
                          >
                            {teacher.isMasked ? (
                  ...
                  {/* Unverified Accounts */}
                  {unverifiedData.map((teacher) => (
                    <TableRow key={teacher.id} className="bg-gray-50/30 dark:bg-gray-800/10">
                      <TableCell className="px-5 py-4 sm:px-6 text-start">
                        <div className="flex items-center gap-3">
                          <div>
                            <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
            ...
            <form className="flex flex-col" onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
              ...
              <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
                ...
                <Button size="sm" type="submit" disabled={editLoading}>
                  {editLoading ? "Saving changes in progress" : "Save Changes"}
                </Button>
              </div>
            </form>
          </div>
