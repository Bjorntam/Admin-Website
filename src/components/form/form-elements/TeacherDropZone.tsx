...
const DropzoneComponent: React.FC = () => {
  ...
  const registerSingleTeacher = async (teacher: CsvRow) => {
    try {
      // Check if fields are valid
      if (!teacher.email || !teacher.teacherName || !teacher.gradeLevel|| !teacher.subject) {
      ...
      const teacherRef = doc(db, "teachers", teacher.email.toLowerCase());
  ...
  const handleBulkRegister = async () => {
    ...
    for (let i = 0; i < csvData.length; i++) {
      const row = csvData[i];
      if (!row.email || !row.teacherName || !row.gradeLevel|| !row.subject) {
        setAlertInfo({
          type: "error",
    ...
    for (const row of csvData) {
      const result = await registerSingleTeacher(row);
      if (result.success) {
        successCount++;
      } else {
        failureCount++;
            ...
            <div className="flex justify-between items-start">
              <div>
                <h4 className="text-2xl font-semibold text-gray-800 dark:text-white/90">
                  Review Teacher's Bulk Registration Details
                </h4>
                      ...
                      <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">
                        {row.isEditing ? (
                          <input
                            ...
                            onChange={(e) => updateRow(index, "teacherName", e.target.value)}
                            className="w-full p-1 border rounded outline-none border-gray-300 dark:border-gray-700 bg-transparent"
                          />
                        ) : (
                          <span className="w-full p-1 outline-none border-gray-300 dark:border-gray-700 bg-transparent">{row.teacherName}</span>
                        )}
                      </TableCell>
                      <TableCell className="px-4 py-3 text-gray-800 text-start text-theme-sm dark:text-white/90">
                        {row.isEditing ? (
